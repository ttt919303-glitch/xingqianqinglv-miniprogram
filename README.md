...
- 物品清单：按证件、电子产品、衣物、旅行用品、洗漱用品、药品分类统计准备进度，并提供短途、海边、亲子、商务、露营等推荐物品库。
- 分类详情：支持全部、未打包、已打包筛选，可勾选准备状态，可新增和删除自定义物品。
- 旅行详情：聚合行程路线、交通卡片、旅行备忘、账单预算、地点收藏和行李清单入口，模块卡片可直接跳转；旅行备忘支持新增、编辑、完成、删除和关联景点/交通。
- 行程计划：展示交通方式、交通时间、交通耗时和当天景点计划。
- 路线编辑：支持时间优先、少换乘、距离优先三种本地推荐策略，并可用上移/下移调整景点顺序；每段路线可选择步行、地铁、打车、骑行，自动重算时间、预算和地图预览。
...
- `npm run test`：验证新增物品、新增行程、路线排序、分段交通方式、地图点位、交通卡片、正式备忘、账单编辑删除、地点收藏、推荐物品库、旅行详情、腾讯地图请求构造、数据重置等核心逻辑。
